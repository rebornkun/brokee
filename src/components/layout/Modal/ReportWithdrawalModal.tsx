...
import { reportWithdrawal } from "../../../services/withdrawal/withdrawal.service";
import { useAppStore } from "../../../store/store";

const ReportWithdrawalModal = () => {
  ...
  const {
    mutate: reportWithdrawalMutate,
    isPending: reportWithdrawalIsLoading,
  } = useMutation({
    mutationKey: [MutationKeys.REPORTWITHDRAWAL],
    mutationFn: () => reportWithdrawal(modalData[0], message),
    onSuccess: (data) => {
      setModalIsOpen(false);
      setIsDrawerOpen(false);
      setModalData([]);
      queryClient.invalidateQueries({
        queryKey: [`${QueryKeys.GETWITHDRAWALSTABLEDATA}`],
      });
    },
    onError: (error) => {
      // console.log(error);
      ...
      <div className="flex flex-col w-full h-full items-center justify-between p-[30px] ">
        ...
        <div className="flex items-center justify-center w-full gap-[10px] ">
          ...
          <Button
            ...
            loading={reportWithdrawalIsLoading}
            onClick={() => {
              reportWithdrawalMutate();
            }}
          >
            Report
          </Button>
...
export default ReportWithdrawalModal;
